...
- 每次功能交付后，更新 `docs/YYYY-MM-DD/功能点.md`。

## HackTravel 项目约束

- 匿名优先可用，登录入口默认关闭。
- 主模型为 Codex 5.4，备路由为硅基流动、NVIDIA NIM。
- 主备全失败时必须回退缓存路线。
- 所有行程字段需经过 Pydantic 校验与归一化。
- 错误码前缀统一为 `HKT`。
- 日志包含 `request_id` 以便追踪。
